...
Aus der Kettenregel in Satz {prf:ref}`satz:produktregel` können wir ein wichtiges Werkzeug für die Integralrechnung ableiten, die sogenannte _Substitutionsregel_.
...
````{prf:theorem} Substitutionsregel
...
````{prf:remark}
:label: bem:substitution_einfach
...
Dennoch haben wir Glück, dass ein Vielfaches der Ableitung im Integranden vorkommt, nämlich der Faktor $x$, und somit können wir die Substitutionsregel für die Integralrechnung aus Satz {prf:ref}`satz:substitutionsregel` anwenden.
...
$iii)$ Zuletzt wollen wir das verbliebene Integral aus Beispiel \ref{bsp:partielle_integration} mit Hilfe der Substitutionsregel berechnen.
...
\begin{split}
  ...
  \end{split}
\end{equation*}
Insgesamt erhalten wir also für das Integral der Arkussinus Funktion in Beispiel {prf:ref}`bsp:partielle_integration`:
\begin{equation*}
